...
const todayKey = () => `lexi-daily-${new Date().toISOString().slice(0, 10)}`;
...
export default function DailyScreen() {
  ...
  const [phase, setPhase] = useState<'loading' | 'already' | 'playing' | 'finished'>('loading');
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && localStorage.getItem(todayKey()) === 'done') {
      setPhase('already');
      return;
    }
  ...
  const choose = (opt: string) => {
    ...
    setTimeout(() => {
      if (!correct) {
        setPicked(null); // буруу бол дахин оролдоно
        return;
      }
          ...
          <>
            ...
            <View style={styles.options}>
              {current.options.map((opt) => {
                ...
                const showCorrect = isPicked && isAnswer;
          ...
          <View style={styles.resultCard}>
            ...
            <View style={styles.rewardChips}>
              <View style={styles.chipPeach}>
                <Text style={styles.chipPeachText}>+{REWARD.exp} EXP</Text>
              </View>
              <View style={styles.chipSand}>
                <Text style={styles.chipSandText}>+{REWARD.coins} 🪙</Text>
              </View>
            </View>
